...
- Рекомендуется установить [`pre-commit`](https://pre-commit.com/#install) для поддержания проекта в адекватном состоянии.
  ...
  - Отформатировать код в соответствии с принятым стилем можно выполнив следующую команду в корне вашего проекта:
    ...
    ```
- Ссылка на свой `fork` репозитория размещается в [таблице](https://docs.google.com/spreadsheets/d/1FxIaIDetRnTDKW5Z1Q_4Y8ctyB5_spviLxrav_cJUMY/edit?usp=sharing) курса с результатами.
...
- Если `review` запрошено _после_ **дедлайна** и ваша работа **зачтена**, то вы получаете **половину полного балла за домашнюю работу**, при условии, что первый запрос `review` был до дедлайна.
...
Оценка за курс складывается из баллов, полученных за работу в семестре, и баллов за доклад.
...
Пусть за летучки получено 1 и 0.5 баллов. Пусть за доклад было получено 8 баллов. Тогда оценка за курс: $((5 + 5) * 1 + (1 + 2) * 0.5) * (8 / 10) = 9.2$
